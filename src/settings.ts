...
export class SettingsTab extends PluginSettingTab {
	...
	display(): void {
		...
		new Setting(containerEl)
			.setName('Replace displayed titles with first heading')
			.setHeading()

		// replaceTabs
		new Setting(containerEl).setName('replaceTabs').addToggle((toggle) =>
			toggle.setValue(settings.setTabTitles).onChange(async (v) => {
				settings.setTabTitles = v
				await saveSettings(this.plugin)
				settings.setTabTitles
					? tabTitles.enable()
					: tabTitles.disable()
			})
		)
		...
		new Setting(containerEl)
			...
			.addToggle((toggle) =>
				toggle
					.setValue(settings.replaceExplorer)
					.onChange(async (v) => {
		...
		// replaceGraph
		new Setting(containerEl).setName('replaceGraph').addToggle((toggle) =>
		...
		// replaceSuggeest
		new Setting(containerEl).setName('replaceSuggest').addToggle((toggle) =>
		...
		new Setting(containerEl)
			...
			.addToggle((toggle) =>
				toggle
					.setValue(settings.replaceBacklink)
					.onChange(async (v) => {
