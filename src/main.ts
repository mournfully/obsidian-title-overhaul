import { Plugin, Events, EventRef } from 'obsidian'
...
export default class HeadingsOverhaulPlugin extends Plugin {
	...
	async onload(): Promise<void> {
		...
		app.workspace.onLayoutReady(async () => {	
			...
			if (settings.setTabTitles) await tabTitles.enable()
		})

		this.addCommand({id: 'test-command', name: 'command for executing test functions', callback: () => { testCommand() }})
		console.log(`Enabled HeadingsOverhaulPlugin: "${Date.now()}"`)
	}

	async onunload(): Promise<void> {
		await tabTitles.disable()
